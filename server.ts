...
const openai = new OpenAI('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');
...
io.on('connection', async socket => {
    ...
    socket.on("compile-code", async (code) => {
        ...
        socket.on("compile-Ai", async (code) => {
            await createTempFileWithCode(code);
    
            await createExectuableFile()
                .then(res => socket.emit("compile-Ai-msg", res))
                .catch(async err => {
                    console.log(err);
                    ...
                    const completion = await openai.complete({
                        ...
                        prompt: `explain the error in simple terms and show the line number where the error occured: ${err}`,
                    ...
                    socket.emit('compile-Ai-msg', choices[0].text);
